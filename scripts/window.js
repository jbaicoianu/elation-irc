elation.require(['ui.window', 'irc.messages', 'irc.nicklist', 'irc.input'], function() {

  elation.component.add("irc.window", function() {
    this.init = function() {
  //console.log(elation.irc.window.extendclass);
      elation.irc.window.extendclass.init.call(this);
      //this.initUIWindow();
      ...
      //this.contentarea = elation.html.create({tag: "div", append: this.content, classname: 'elation_irc_window_content'});
      elation.html.addclass(this.content, 'elation_irc_window_content');
      if (this.channel.name[0] == '#') {
        this.messages = elation.irc.messages(null, elation.html.create({append: this.content}), {window: this, group: true});
        this.nicklist = elation.irc.nicklist(null, elation.html.create({append: this.content}), {window: this});
      } else {
        this.messages = elation.irc.messages(null, elation.html.create({append: this.content}), {window: this, group: false});
      }
